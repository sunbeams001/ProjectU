...
### PixivisionApi - Pixivision 特辑相关 API (2/2) ✅

| 功能 | 端点 | 实现 | 测试 | DTO状态 | 备注 |
|-----|------|------|------|---------|------|
| 获取文章列表 | `/{lang}/c/{category}` | ✅ | ✅ | ✅ | 通过HTML解析，支持分页 |
| 获取文章详情 | `/{lang}/a/{articleId}` | ✅ | ✅ | ✅ | 通过HTML解析，获取完整文章内容 |

**特别说明**:
- **数据来源**: Pixivision是Pixiv的附属网站 (https://www.pixivision.net/)
- **数据格式**: 返回HTML，使用Ksoup解析
- **支持类别**: illustration(插画特辑)、manga(漫画特辑)
- **支持语言**: zh, zh-tw, en, ja, ko
- **分页支持**: 通过 `?p={page}` 参数实现分页加载
...
### 2026-01-10
- ✅ **PixivisionApi 模块**: 新增 Pixivision 特辑 API 模块（2个方法）
- 📄 **HTML 解析**: 使用 Ksoup 解析 Pixivision 网站 HTML 内容
- 🌐 **多语言支持**: 支持简体中文、繁体中文、英语、日语、韩语
- 📊 **特辑类别**: 支持插画特辑和漫画特辑
- 📝 **文档更新**: 更新 API_STATUS.md、API测试工具使用指南.md、README.md
