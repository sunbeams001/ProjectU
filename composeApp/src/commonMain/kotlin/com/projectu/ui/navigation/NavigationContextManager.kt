 ...
 * 导航上下文管理器
 * 
 * 管理作品详情页的导航上下文，解决 Screen 参数不可序列化的问题
 * 
 * 使用方式：
 * 1. 推送页面前，调用 createContext() 创建上下文并获取 key
 * 2. 将 key 传递给 ArtworkDetailScreen
 * 3. 详情页通过 key 获取上下文，订阅 listSource.artworkIdsFlow
 * 4. 页面销毁时，调用 removeContext() 清理
 * 
 * Activity 状态恢复时：
...
object NavigationContextManager {
    ...
    fun createContext(
        ...
        contexts[key] = ArtworkDetailContext(
